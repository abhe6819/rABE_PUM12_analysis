...
# rename columns
colnames(PUM2_bedClosest) <- c("rABE_chrom","rABE_start", "rABE_stop", "rABE_ID", "rABE_ogrow", "rABE_strand", "eCLIP_chrom","eCLIP_start", "eCLIP_stop", "eCLIP_ID", "eCLIP_splitID", "eCLIP_strand", "Distance_to_motif","Ts", "Window_size", "within_Window", "ddG", "ss_ddG","numAdjMotif", "diffratios", "padj", "avgExonDP")
...
filter(PUM2_bedClosest, within_Window=='a', diffratios > 0.004, numAdjMotif<=0, ss_ddG<10) %>% 
...
ddg_Fit <- glm(diffratios ~ ddG+ Ts, data = filter(PUM2_bedClosest, within_Window=='a', diffratios > 0.004, numAdjMotif<=0, ss_ddG < 10))
summary(ddg_Fit)
ddg_Fit <- lm(diffratios ~ ddG+ as.factor(Ts)+ numAdjMotif+ss_ddG+avgExonDP+Distance_to_motif, data = filter(PUM2_bedClosest, within_Window=='a'))
summary(ddg_Fit)
#R^2
with(summary(ddg_Fit), 1 - deviance/null.deviance)
y <- filter(PUM2_bedClosest, within_Window=='a', diffratios > 0.004, numAdjMotif<=0, ss_ddG < 5)
...
bedtools getfasta -fi genomeFiles/hg38.fa -bed PUM2_analysis/window10nt_PUM12_deaminase_dataPUM2-rABE_cmh_A-G-doxH_unfiltered.sorted.bed -fo PUM2_analysis/window10nt_PUM12_deaminase_dataPUM2-rABE_cmh_A-G-doxH_unfiltered.sorted.fa -rna
