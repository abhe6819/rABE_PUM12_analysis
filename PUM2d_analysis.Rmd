...
summary(ddg_Fit)
ddg_Fit <- glm(diffratios ~ ddG+ as.factor(Ts)+ numAdjMotif+ss_ddG+avgExonDP+Distance_to_motif, data = filter(PUM2_bedClosest, within_Window=='a'))
summary(ddg_Fit)
#R^2
...
summary(ddG_lmModel)
ddG_lmFineModel <- lm(diffratios ~ numAdjMotif+ddG+avgExonDP+Ts+Distance_to_motif, data = PUM2_train[,c("Distance_to_motif","Ts","ddG","ss_ddG","numAdjMotif","diffratios","avgExonDP")])
